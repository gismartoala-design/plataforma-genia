...
async function generateIT() {
  ...
  // 1. Find the module
  const modules = await db.select().from(schema.modulos).where(eq(schema.modulos.id, 35));
  if (modules.length === 0) {
    console.log('Module not found');
    process.exit(1);
  }
  const module = modules[0];
  console.log(`Module ID: ${module.id} - ${module.nombreModulo}`);

  // 2. Find levels
  const levels = await db.select().from(schema.niveles).where(eq(schema.niveles.moduloId, module.id));
  console.log(`Found ${levels.length} levels for this module.`);
  
  const level4 = levels.find(l => l.orden === 4);
  const level7 = levels.find(l => l.orden === 7);

  if (!level4 || !level7) {
    console.log('Level 4 or 7 not found');
    process.exit(1);
  }

  // The template content format based on ItViewer's properties
  const itLevel4Fases = {
    identificacion: {
        codigo: "IT-PEC-L4",
        mes: "Mes 2",
    ...
    fase2Filas: [
        ...
        },
        {
            ...
            opcionA: { nombre: "Eventos Independientes Completos", ventajas: "Estadística pura", desventajas: "Puede frustrar al jugador (mala experiencia de usuario si falla repetidamente)" },
  ...
  const itLevel7Fases = {
    ...
    fase2Filas: [
        {
            criterio: "Manejo de Probabilidades Extremadamente Pequeñas",
            opcionA: { nombre: "Multiplicación Directa Normal", ventajas: "Traducción fiel de la fórmula matemática al código", desventajas: "Underflow System Error (Probabilidad se aproxima a coma flotante infinita = O.0)" },
            opcionB: { nombre: "Uso de Logaritmos (Log-Probability)", ventajas: "En lugar de multiplicar decimales, suma enteros. Evita el underflow computacional", desventajas: "Agrega abstracción matemática extra en el código" }
        },
        {
            criterio: "Manejo de Probabilidad Cero (Laplace)",
            opcionA: { nombre: "Cero Absoluto (0%) para palabras no vistas", ventajas: "Ninguna", desventajas: "Anula (multiplica por cero) el score entero del documento si hay 1 palabra nueva" },
  ...
  // Insert or Update Level 4
  const exist4 = await db.select().from(schema.plantillasIt).where(eq(schema.plantillasIt.nivelId, level4.id));
  if (exist4.length > 0) {
    await db.update(schema.plantillasIt).set({
      ...
      fases: itLevel4Fases
    }).where(eq(schema.plantillasIt.id, exist4[0].id));
    console.log(`Updated IT Level 4`);
  } else {
    await db.insert(schema.plantillasIt).values({
      nivelId: level4.id,
      titulo: 'Simulador Estocástico de Recompensas',
      descripcion: 'Aplicación de Variables Aleatorias y Simuladores Numéricos Estocásticos.',
      fases: itLevel4Fases
    });
    console.log(`Inserted IT Level 4`);
  }

  // Insert or Update Level 7
  const exist7 = await db.select().from(schema.plantillasIt).where(eq(schema.plantillasIt.nivelId, level7.id));
  if (exist7.length > 0) {
    await db.update(schema.plantillasIt).set({
      ...
      fases: itLevel7Fases
    }).where(eq(schema.plantillasIt.id, exist7[0].id));
    console.log(`Updated IT Level 7`);
  } else {
    await db.insert(schema.plantillasIt).values({
      nivelId: level7.id,
      titulo: 'Motor Híbrido Clasificador Bayesiano',
    ...
    console.log(`Inserted IT Level 7`);
  }

  process.exit(0);
}

generateIT().catch(console.error);
