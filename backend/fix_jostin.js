...
// When running inside 'backend', .env is in the current directory
...
async function fixJostin() {
    ...
    try {
        ...
        await client.query(`UPDATE cursos SET profesor_id = $1 WHERE id = ANY($2::int[])`, [userId, courseIds]);
        console.log(`Assigned courses: ${courseIds.join(', ')}`);

        // 4. Assign Institutional Modules as profesor_id
        await client.query(`UPDATE modulos_inst SET profesor_id = $1 WHERE curso_id = ANY($2::int[])`, [userId, courseIds]);
