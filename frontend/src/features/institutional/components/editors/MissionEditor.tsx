                              ...
                              <div className="space-y-3">
                                {(moment.student?.options || [{text: ""}, {text: ""}]).map((opt: any, i: number) => (
                                  <div key={i} className="flex items-center gap-3 group animate-in slide-in-from-left-2 duration-300">
                                      ...
                                      />
                                      { (moment.student?.options?.length > 2) && (
                                        <Button 
                                          variant="ghost" 
                                          size="icon" 
                                          onClick={() => {
                                            const newOpts = moment.student.options.filter((_: any, index: number) => index !== i);
                                            // Adjust correct answer if needed
                                            if (moment.student.correct_answer === i) {
                                              handleUpdateMoment(idx, 'student', 'correct_answer', null);
                                            } else if (moment.student.correct_answer > i) {
                                              handleUpdateMoment(idx, 'student', 'correct_answer', moment.student.correct_answer - 1);
                                            }
                                            handleUpdateMoment(idx, 'student', 'options', newOpts);
                                          }}
